...
use abi_stable::{
    std_types::{RHashMap, RCowStr, RCowSlice, RVec, Tuple2},
    rvec,
    StableAbi
};
...
impl<'value> Value<'value> {
    ...
    pub fn into_static(self) -> Value<'static> {
        match self {
            Self::String(s) => Value::String(RCowStr::Owned(s.to_string())),
            Self::Array(arr) => arr.into_iter().map(Value::into_static).collect(),
            Self::Object(obj) => obj
                .into_iter()
                .map(|Tuple2(k, v)| (RCowStr::Owned(k.to_string()), v.into_static()))
                .collect(),
            Self::Static(s) => Value::Static(s),
            Self::Bytes(b) => Value::Bytes(RCowSlice::Owned(b.to_vec())),
        }
    }
    ...
    pub fn clone_static(&self) -> Value<'static> {
        match self {
            ...
            Self::Object(obj) => obj
                .iter()
                .map(|Tuple2(k, v)| (RCowStr::Owned(k.to_string()), v.clone_static()))
                .collect(),
            Self::Static(s) => Value::Static(*s),
            Self::Bytes(b) => Value::Bytes(RCowSlice::Owned(b.to_vec())),
        }
    ...
    where
        RCowSlice<'value, u8>: Borrow<Q> + Hash + Eq,
        Q: Hash + Eq + Ord,
    {
    ...
    fn arb_tremor_value() -> BoxedStrategy<Value<'static>> {
        let leaf = prop_oneof![
            ...
            any::<u64>()
                ...
                .prop_map(Value::Static),
            any::<Vec<u8>>().prop_map(RCowSlice::from).prop_map(Value::Bytes),
            any::<f64>()
        ...
        leaf.prop_recursive(
            ...
            |inner| {
                prop_oneof![
                    ...
                    prop::collection::hash_map(".*".prop_map(RCowSlice::from), inner, 0..10)
